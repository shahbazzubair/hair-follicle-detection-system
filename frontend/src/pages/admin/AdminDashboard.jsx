...
export default function AdminDashboard() {
    ...
    if (result.isConfirmed) {
      try {
        if (action === "delete") {
          ...
          await axios.put(
            `http://localhost:8000/api/admin/verify-doctor/${userId}`,
            { status },
          );
        }
  ...
  const stats = {
    ...
    pending: users.filter((u) => u.role === "doctor" && u.status === "Pending")
      .length,
  ...
  const renderContent = () => {
    if (loading)
      return <div className={styles.loader}>Syncing with Database...</div>;
      ...
      return (
        <>
          <div className={styles.tabContainer}>
            <button
              className={
                activeTab === "doctors" ? styles.activeTabBtn : styles.tabBtn
              }
            ...
            <button
              className={
                activeTab === "patients" ? styles.activeTabBtn : styles.tabBtn
              }
                          ...
                          {user.role === "doctor" &&
                            user.status === "Pending" && (
                              <>
                                <button
                                  onClick={() =>
                                    handleAction(user.id, "verify", "Approved")
                                  }
                                ...
                                <button
                                  onClick={() =>
                                    handleAction(user.id, "verify", "Rejected")
                                  }
            ...
            <div className={styles.barContainer}>
              ...
              <div className={styles.barBG}>
                <div
                  ...
                  style={{
                    width: `${(stats.patients / stats.total || 0) * 100}%`,
              ...
              <div className={styles.barLabel}>
                Verified Doctors ({stats.verified})
              </div>
              <div className={styles.barBG}>
                <div
                  className={styles.barFill}
                  style={{
                    width: `${(stats.verified / stats.total || 0) * 100}%`,
    ...
    <div className={styles.adminContainer}>
      <aside className={styles.sidebar}>
        ...
        <nav className={styles.nav}>
          <button
            className={
              view === "management" ? styles.navItemActive : styles.navItem
            }
          ...
          <button
            className={
              view === "analytics" ? styles.navItemActive : styles.navItem
            }
          ...
          <button
            className={
              view === "settings" ? styles.navItemActive : styles.navItem
            }
